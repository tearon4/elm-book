...
ElmはFRPでGUIアプリケーションを記述する言語としてEvan Czaplickiが発表したのが始まりです。その後、沢山のユーザーが使い意見を出し合い変革を経て、FRPはElm-Architectureというパラダイムに変わり、今はElm-Architectureでブラウザアプリケーションを記述、生成する言語として開発が進められています。
...
後述しますが、Elm-Architectureというパラダイムにそってアプリケーションを書くので、Elmはプログラミング言語であり、フレームワークでもあります。
...
###Elm-Architectureというパラダイム

GUIアプリケーションを作るのに、MVCやMVVMなどの設計方針がありますが、ElmではElm-Architectureというもので書きます。
...
Elm-Architectureは他のJSフレームワーク（redux、Cycle.js）に影響を与えてもいる考え方です。
