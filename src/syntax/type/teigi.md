...
type 型名 = データ構築子 （定義に必要な既存の型）... | データ構築子 （定義に必要な既存の型） | ...

type alias 新しい型名 = 既存の型

type alias 新しいレコード型名 = {
    プロパティ名
  , プロパティ名
  , ...
}
...
`type alias`は既存の型に別の型名を付けるときに使います。また「レコード型」を定義するときも特別にこちらを使います。
...
上記の場合このBool型は、TrueかFalseどちらかになる。という意味になります。フラグのイメージそのままです。
...
Union typeでは、自分自身が定義に表れるような再帰した型を定義できます。
